...
func (s *stepGrabPublicIP) Run(_ context.Context, state multistep.StateBag) multistep.StepAction {
	...
	if err != nil {
		err := fmt.Errorf("[ERROR] Failed to fetch Public IP address for instance '%s'", instanceID)
		state.Put("error", err)
		ui.Error(err.Error())
	...
	if err != nil {
		err := fmt.Errorf("[ERROR] Failed to write ip address to file %s", hostsFilePath)
		state.Put("error", err)
		ui.Error(err.Error())
		return multistep.ActionHalt
	}
...
func (s *stepGrabPublicIP) Cleanup(state multistep.StateBag) {}
