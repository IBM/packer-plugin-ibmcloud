...
func (self SoftlayerClient) doHttpRequest(path string, requestType string, requestBody *bytes.Buffer) ([]interface{}, error) {
	...
	if err != nil {
		err := errors.New(fmt.Sprintf("Failed to get proper HTTP response from SoftLayer API. Error: %s", err))
		return nil, err
	}
...
func (self SoftlayerClient) doModifiedHttpRequest(path string, requestType string, requestBody *bytes.Buffer) ([]interface{}, error) {
	responseBody, err := self.doRawHttpRequest(path, requestType, requestBody)
	if err != nil {
...
func (self SoftlayerClient) GrabCredentials(instanceID string, state multistep.StateBag) (string, string, error) {
	...
	for i := 0; i < attemptCount; i++ {
		data, err := self.doRawHttpRequest(fmt.Sprintf("SoftLayer_Virtual_Guest/%s/getObject.json?objectMask=mask[id,operatingSystem[id,passwords[username,password]]]", instanceID), "GET", nil)
		//ui.Say(string(data))
	...
	return "", "", errors.New(fmt.Sprintf("Unable to obtain password after %v seconds", int(waitDuration)*int(attemptCount)))
}
...
//
func (self SoftlayerClient) copyImageToDatacenters(imageId string, datacenterId []string) error {
	...
	requestBody, err := self.generateRequestBody(addLocationsList)
	if err != nil {
		return err
	}

	data, err := self.doModifiedHttpRequest(fmt.Sprintf("SoftLayer_Virtual_Guest_Block_Device_Template_Group/%s/addLocations.json", imageId), "POST", requestBody)
	if err != nil {
		return err
	}
